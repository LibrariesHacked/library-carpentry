...
One way of approaching this is to use a Pivot table to group the data by branch.  Then calculate a sum of issues for each branch.
...
5. Then drag the Issues field into the Values section
...
3. In Excel, the **'YEAR()'** function in a formula extracts the year from a date.  However, we don't have valid dates, we have data as a year and month in column B ('2008-04').  We can write a formula that will convert that data into a full date (e.g. '2008-04-01'), then extract the year.  In the first data row of the new column enter the formula: **=YEAR(CONCATENATE(B2,"-01"))**
4. This is combining two functions: CONCATENATE and YEAR.  CONCATENATE adds '-01' to the data, to make it into a full date.  It then uses a function called YEAR which gives us just the year.  The year should then apear as 2008.
5. Use **'Fill Down'** in Excel to copy the formula for all cells of the column. Hint: Highlight the cell with the formula and use the keyboard shortcut **Ctrl+Shift+End**.  Then on the Excel menu select **Fill Down**.
5. Repeat the pivoting process to group the data by the new Year column instead of Library.

### Google Sheets

1. Start a new sheet and import the data, or use the previous sheet.
2. Create a new column and call it 'Year'
3. Google sheets will already understand the month column is representing a Date value (they will add a day value of 01).  In the first data row of the new Year column enter the formula: **=YEAR(B2)**
...
This is similar to the previous question, but we need to extract year instead of the month.  We also need to determine which calendar month is the busiest.  One month (e.g. January) could be busiest one year (most enquiries), but less busy the nest.  We can calculate an average for each calendar month across all years.

### Excel

1. Within Excel go back to the main data sheet.
2. Add a new Column called **'Month Number'**.
3. We could do the same as the previous question, but using the 'Month' function.  However, lets try something different.  We know to get the month we just need the last two numbers from the year/month field, as that field is always in the format '2008-04'.  Create a formula but use: **=RIGHT(B2,2)**
4. This will take the two numbers from the right hand side to give a month.
5. Repeat the process as before to pivot the data.  Filter to only include City library drag the Library field into the Filter section.  On the pivot table you can then change from 'All' to 'City'.
